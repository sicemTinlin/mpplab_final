...
function [west,Aest,erro]=esprit(y,M,Ma)
...
% NOTE: west*n = w*d so w = west*(n/d)
  ...
  x=hankel(y);     %make a hankel matrix out of the data
  ...
  D=real(eig(Rx)); %find the real eigenvalues of Rx
  ...
  [V,D]=eig(Rx);    % find the generalized eigenvalues and eigenvectors of ASA' (also called Cxx in cisoids and noise paper)
